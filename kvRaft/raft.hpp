...
public:
    ...
    string snapShot;//文件块，因为很小，所以不考虑分块传输
...
//记录当前client对server命令的情况
class Operation{
public:
    string getCmd();//只记录clent的情况
    string op;//操作
    string key;
    string value;
    int clientId;//客户端id
    int requestId;//客户端的请求id
    int term;//服务器当前term和index
    int index;
};
...
public:
    ...
    int m_cmdIndex;//命令在server中应用时的index和term
    int m_curTerm;
    bool isLeader;//是否时leader
};
//同应用层交互的需要提交到应用层并apply的封装成applyMsg的日志信息
//用来代表待应用的日志，需要记录这次应用是已经提交的日志，并且记录这次提交的日志命令内容和下标
class ApplyMsg {
public:
    bool commandValid;//命令是否正确
	string command;//命令内容
    int commandIndex;//命令在server中的index和term
    int commandTerm;
    Operation getOperation();//得到命令的相关信息
//快照相关
    int lastIncludedIndex;
    int lastIncludedTerm;
    string snapShot;
...
Operation ApplyMsg::getOperation(){ //将string命令转化为operate格式
    Operation operation;
    vector<string> str;
    string tmp;
...
class PeersInfo{
public:
    pair<int, int> m_port;//端口
    int m_peerId;//id
    bool isInstallFlag;//是否安装快照
};
//日志
class LogEntry{
public:
    LogEntry(string cmd = "", int term = -1):m_command(cmd),m_term(term){}
    string m_command;//日志具体内容
    int m_term;//当前term
};
//持久化类
class Persister{
public:
    vector<LogEntry> logs;//日志
    int cur_term;//当前term
    int votedFor;//投票结果
//快照相关
    string snapShot;
...
public:
    ...
    //     m_sendLogs.clear();
    // }
    int m_term;//Leader 的 Term ID
    int m_leaderId;//Leader 的 ID
    int m_prevLogIndex;// 在正在备份的日志记录之前的日志记录的 index 值
    int m_prevLogTerm;//在正在备份的日志记录之前的日志记录的 Term ID
    int m_leaderCommit;//Leader 已经提交的最后一条日志记录的 index 值
    string m_sendLogs;//正在备份的日志记录
...
public:
    int m_term;//接收方的当前 Term ID
    bool m_success;//投票结果
    int m_conflict_term;//冲突条目的任期
    int m_conflict_index;//该任期下第一条条目的索引
};
//用于发送选举信息
class RequestVoteArgs{
public:
    int term;//candidate的任期号
    int candidateId;//发起投票的candidate的ID
    int lastLogTerm;//candidate的最高日志条目索引
    int lastLogIndex;//candidate的最高日志条目的任期号
};

class RequestVoteReply{
public:
    int term;//服务器的当前任期号，让candidate更新自己
    bool VoteGranted;//如果是true，意味着candidate收到了选票
};
...
private:
    locker m_lock;//交互锁
    cond m_cond;//条件变量
    vector<PeersInfo> m_peers;//所有服务器信息
    Persister persister;//持久化量
    int m_peerId;//服务器id
    int dead;//服务器死活

    //需要持久化的data
    int m_curTerm;//服务器知道的最近任期，当服务器启动时初始化为0，单调递增
    int m_votedFor;//当前任期中，该服务器给投过票的candidateId，如果没有则为null
    vector<LogEntry> m_logs;//日志条目；每一条包含了状态机指令以及该条目被leader收到时的任期号
    int m_lastIncludedIndex;        //新增的持久化变量，存上次快照日志截断处的相关信息
    int m_lastIncludedTerm;         //新增的持久化变量，存上次快照日志截断处的相关信息
    //易失性状态
    vector<int> m_nextIndex;//针对所有的服务器，内容是需要发送给每个服务器下一条日志条目索引号(初始化为leader的最高索引号+1)
    vector<int> m_matchIndex;//针对所有的服务器，内容是已知要复制到每个服务器上的最高日志条目号，初始化为0，单调递增
    int m_lastApplied;//应用到状态机的最高日志条目索引号，一开始为0，单调递增
    int m_commitIndex;//已知被提交的最高日志条目索引号，一开始是0，单调递增

    // unordered_map<int, int> m_firstIndexOfEachTerm;
    // vector<int> m_nextIndex;
    // vector<int> m_matchIndex;

    int recvVotes;//已获取到的票数
    int finishedVote;//总的投票数
    int cur_peerId;//当前通信的服务器id

    RAFT_STATE m_state;//当前的状态
    int m_leaderId;//记录leader的id
    struct timeval m_lastWakeTime;//上次被唤醒时间
    struct timeval m_lastBroadcastTime;//上次广播时间
...
void Raft::Make(vector<PeersInfo> peers, int id){
    ...
    readRaftState();//读取持久化量
    installSnapShotTokvServer();//安装快照
    ...
    while(1){
        while(!raft->dead){
            ...
            for(int i = 0; i < msgs.size(); i++){
                ...
                //     raft->m_peerId, raft->m_lastApplied, raft->m_commitIndex);
                raft->waitRecvSem();//等待kv层处理完之后才能发送另一条指令
...
void* Raft::callRequestVote(void* arg){
    ...
    args.lastLogIndex = raft->lastIndex();//如果刚进行快照，使用递增或者快照保存信息
...
bool Raft::checkLogUptodate(int term, int index){  //检查有没有投票的条件
...
void* Raft::sendInstallSnapShot(void* arg){
    ...
    for(int i = 0; i < raft->m_peers.size(); i++){
        ...
        clientPeerId = i;
        raft->isExistIndex.insert(i);//记录了插入快照的索引
...
InstallSnapSHotReply Raft::installSnapShot(InstallSnapShotArgs args){
    ...
    if(args.lastIncludedIndex <= m_lastIncludedIndex){//                                                    这里可能会有死循环
        m_lock.unlock();
        return reply;
    }else{
...
int Raft::lastIndex(){
    return m_lastIncludedIndex + m_logs.size();//index叠加进行计算
}

int Raft::lastTerm(){
    int lastTerm = m_lastIncludedTerm;
    if(m_logs.size() != 0){
        lastTerm = m_logs.back().m_term;//最新的term或者快照的term
    }
    return lastTerm;
}
