...
void shardClerk::Join(unordered_map<int, vector<string>>& servers){   //用于创建新的副本组
    JoinArgs args; //join副本组的信息
    args.getServersShardInfoFromMap(servers);   //将map信息转化到类内的string中使得RPC能够正确传输
    args.requestId = getCurRequestId();   //得到当前的requestId
    args.clientId = clientId;
    int cur_leader = getCurLeader();   //得到当前的cur_leader
    ...
    while(1){
        buttonrpc client;
        client.as_client("127.0.0.1", serverPorts[cur_leader][curPort]);//通过接听端口向leader通信
        JoinReply reply = client.call<JoinReply>("Join", args).val();  //得到join结果
...
Config shardClerk::Query(int num){  //回应最新的配置
    QueryArgs args;
    args.configNum = num;
    args.clientId = clientId;
    ...
    while(1){
        ...
        if(!reply.isWrongLeader){
            // printf("Query is finished, port is %d, leader is %d\n", serverPorts[cur_leader][curPort], cur_leader);
             Config retCfg = reply.getConfig(); //string到map的转换
            //  printConfig(retCfg);
             return retCfg;
        }
...
void shardClerk::Leave(vector<int>& gIds){ //从kvsever中删除某些服务器
    LeaveArgs args;
...
void shardClerk::Move(int shard, int gId){ //将某个服务器移动到某个group中
    MoveArgs args;
    args.clientId = clientId;
    args.requestId = getCurRequestId();
    args.getStringOfShardAndGroupId(shard, gId);//格式转换
    int cur_leader = getCurLeader();
