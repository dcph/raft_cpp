...
public:
    ...
    int configNum;//表示该配置的配置号
    vector<int> shards;//表示分片分配情况
    unordered_map<int, vector<string>> groups;//即负责当前分片的集群，以及集群内所有server的信息
};

class JoinArgs{
public:
    void getServersShardInfoFromMap(unordered_map<int, vector<string>> servers);//map转string
...
class LeaveArgs{
public:
    void getGroupIdsInfoFromVector(vector<int>& gIds);//vector转string
    string groupIdsInfo;
    int clientId;
    int requestId;
