...
private:
    ...
    unordered_map<int, int> leaderId;//保存几个group的leader
    int clientId;
    int requestId;
};
...
string Clerk::get(string key){
    ...
    while(1){
        int shard = key2shard(key);//计算key所属的分片
        int gid = config.shards[shard]; //分片对应的组
        if(config.groups.count(gid)){
            printf("in get, key : %s -> gid is %d\n", key.c_str(), gid);
            vector<string> servers = config.groups[gid];//找到对应组的所有服务器信息
            for(int i = 0; i < servers.size(); i++){
                int curLeader = (i + leaderId[gid]) % servers.size();
                int port = make_end(servers[curLeader]); //找到对应kv服务器的端口
                buttonrpc client;
                ...
                if(reply.err == ErrWrongGroup){  //该group中没有对应key
                    break;
                }
            }
        }   
        usleep(100000);
        config = masterClerk.Query(-1);    //如果没有找到，就更新配置信息
    }   
}
...
int main(){
    ...
    initStr2PortMap(5);//记录EVERY_SERVER_RAFT*5个端口号
    vector<vector<int>> port = getMastersBehindPort(EVERY_SERVER_RAFT);//记录EVERY_SERVER_RAFT*EVERY_SERVER_RAFT个端口号 //得到shardMaster后半部分端口信息
    ...
    printConfig(config);
    unordered_map<int, vector<string>> a = getJoinArgs(vector<int>{2});//得到对应编号的的group
    clerk.Join(a);
