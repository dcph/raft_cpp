...
#include <unistd.h>//系统服务函数
#include <sys/stat.h>//文件名与文件对应属性
#include <fcntl.h>//用来处理文件
#include <string.h>
#include "locker.h"
...
Master::Master(int mapNum, int reduceNum):m_done(false), m_mapNum(mapNum), m_reduceNum(reduceNum){
    ...
    for(int i = 0; i < reduceNum; i++){
        reduceIndex.emplace_back(i);//直接在容器的末尾构造对象，这样就省去了拷贝的过程。push_back()需要先构造临时对象，再将这个对象拷贝到容器的末尾
    }
}
...
void* Master::waitReduceTask(void* arg){    //void即为不确定类型。不能用来声明实例对象，void*是一个通用指针，可以指向任意类型的指针
//但是void*不能够解引用，在赋值给其他类型指针的时候要进行强制类型转换，删掉void*所指向的变量也需要强制类型转换
//因为编译器本身不知道void*指向的变量的类型
