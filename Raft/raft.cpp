...
void Raft::Make(vector<PeersInfo> peers, int id){
    ...
    m_leaderId = -1;
    m_votedFor = -1;
    gettimeofday(&m_lastWakeTime, NULL);//用来得到精确时间
    ...
    pthread_create(&listen_tid1, NULL, listenForVote, this);  //开启三个监听线程
...
void* Raft::applyLogLoop(void* arg){
    ...
    while(!raft->dead){
        usleep(10000); //微秒级延迟
        raft->m_lock.lock();
        for(int i = raft->m_lastApplied; i < raft->m_commitIndex; i++){ //将已提交的信息全部传送
            /**
...
void* Raft::electionLoop(void* arg){ //进入选举循环
    Raft* raft = (Raft*)arg;
    bool resetFlag = false;
    while(!raft->dead){
        
        int timeOut = rand()%200000 + 200000; //随机超时时间
        while(1){
            usleep(1000);
            raft->m_lock.lock();
            ...
            if(raft->m_state == FOLLOWER && during_time > timeOut){
                raft->m_state = CANDIDATE;            //超时进行状态转换
            }

            if(raft->m_state == CANDIDATE && during_time > timeOut){ //要求选举
                printf(" %d attempt election at term %d, timeOut is %d\n", raft->m_peerId, raft->m_curTerm, timeOut);
                gettimeofday(&raft->m_lastWakeTime, NULL); //重置时间
                resetFlag = true;
                raft->m_curTerm++;
                raft->m_votedFor = raft->m_peerId;
                raft->saveRaftState();  //持久化变量
                ...
                for(auto server : raft->m_peers){
                    ...
                    pthread_create(tid + i, NULL, callRequestVote, raft);  //发送请求
                    pthread_detach(tid[i]);
                    i++;
                }

                while(raft->recvVotes <= raft->m_peers.size() / 2 && raft->finishedVote != raft->m_peers.size()){    //没到半数票或者结束就一直等待
                ...
                if(raft->recvVotes > raft->m_peers.size() / 2){ //选举成leader
                    raft->m_state = LEADER;
...
void* Raft::callRequestVote(void* arg){  //接受选举结果
    Raft* raft = (Raft*) arg;
    buttonrpc client;
    raft->m_lock.lock();
    ...
    client.as_client("127.0.0.1", raft->m_peers[raft->cur_peerId++].m_port.first); //连接到指定的服务器
    ...
    raft->finishedVote++;
    raft->m_cond.signal();
    if(reply.term > raft->m_curTerm){ //如果过时了，那就转成follower
...
bool Raft::checkLogUptodate(int term, int index){    //检查是否投票
    m_lock.lock();
    if(m_logs.size() == 0){
        m_lock.unlock();
        return true;
...
RequestVoteReply Raft::requestVote(RequestVoteArgs args){  //回复投票结果
...
void* Raft::processEntriesLoop(void* arg){   //作为分离出的线程隔一段时间关注一次日志同步
    Raft* raft = (Raft*)arg;
    while(!raft->dead){
        usleep(1000);
        raft->m_lock.lock();
...
void Raft::push_backLog(LogEntry log){  //插入log
    m_logs.push_back(log);
}

void* Raft::sendAppendEntries(void* arg){  //补充日志
    Raft* raft = (Raft*)arg;
    buttonrpc client;
    ...
    int clientPeerId = raft->cur_peerId;
    client.as_client("127.0.0.1", raft->m_peers[raft->cur_peerId++].m_port.second); //链接对应服务器
    ...
    raft->m_lock.unlock();
    AppendEntriesReply reply = client.call<AppendEntriesReply>("appendEntries", args).val(); //调用对应函数传递log
...
AppendEntriesReply Raft::appendEntries(AppendEntriesArgs args){ //接受到之后复制log
...
void Raft::serialize(){ //将容器转化为string
    string str;
...
bool Raft::deserialize(){
    ...
    int length = lseek(fd, 0, SEEK_END); //设置偏置量
    lseek(fd, 0, SEEK_SET);
    char buf[length];
    bzero(buf, length);
...
void Raft::saveRaftState(){  //持久化的相关
    persister.cur_term = m_curTerm;
    persister.votedFor = m_votedFor;
    persister.logs = m_logs;
    serialize();
